...
| BUG-C12 | ✅ | Bulk scenarios generated with custom initial gaps (via SimGenerator) always started at the parameter-file gap values, ignoring per-scenario gaps | `ScenarioTimeline.from_csv` skipped the `Initial gaps:` metadata row (row 2) without reading it; all 4 simulation entry points used `loaded.initial_conditions` unchanged | Added `initial_gap_12_m` / `initial_gap_23_m` fields to `ScenarioTimeline`; `from_csv` parses row 2 when present; `apply_initial_gaps(initial)` overrides the fields; called at all entry points in `simulation.py`, `optimization.py` (×2), `gui.py` | Regression: load a SimGenerator CSV with non-default gaps → simulation starting gap must match metadata row, not parameters file |

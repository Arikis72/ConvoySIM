...
| REQ-21-14 | §21 GUI — Online Visualization with live leader control | "Open Online Visualization" button; live leader control row (Pause, Accelerate/Decelerate with rate fields, Orange/Red/FORT brake, Back to Sim, Save As); 1-click=1s step; ← undo; Play=HOLD auto-advance; Save As merges scenario CSV with updated leader trajectory | ✅ | Manual review pending; `OnlineVisualizationWindow` class in `gui.py`; `LiveSimStepper` in `live_stepper.py` | — |
| REQ-21-15 | §21 GUI — Online Visualization back-step correctness | ← undo removes exactly 1 output-second of live rows per press; scrub then Resume continues from scrubbed position | ✅ | 2026-05-31; `_output_rows_per_second` (was `_steps_per_second`) fixed to use `output_resolution_s`; BUG-C04 | — |
| REQ-21-16 | §21 GUI — Online Visualization Save As preserves pre-live truck events | Truck2/truck3 image events occurring before the live-entry time appear in the saved scenario CSV | ✅ | 2026-05-31; event-only row injection in `_save_online_scenario`; BUG-C05 | — |
| REQ-21-17 | §21 GUI — Online Visualization leader rectangle state colors | Leader upper rectangle: white/black (normal), orange fill (orange brake), red fill (red brake), black fill (FORT) | ✅ | 2026-05-31; `leader_status_style()` using `is_orange_status`/`is_red_status` helpers; `leader_command_kind` propagated through `_build_output_row`; BUG-C06, BUG-C10 | — |
| REQ-21-18 | §21 GUI — Online Visualization auto-advance follows original profile | Auto-advance (Pause/Resume) converges leader velocity to original scenario profile with `return_to_sim_velocity_s` look-ahead (default 3 s, user-tunable in parameters CSV); convergence is gradual (clamped to vehicle accel limits) | ✅ | 2026-05-31; `FOLLOW_PROFILE` command + `return_to_sim_velocity_s` param; velocity ramp via `/ params.return_to_sim_velocity_s`; BUG-C07, BUG-C09 | — |
| REQ-21-19 | §21 GUI — Online Visualization Back to Sim always enabled during live mode | Back to Sim button re-enabled whenever a new intervention is applied, even after ← depleted live rows | ✅ | 2026-05-31; `_ensure_live_mode` short-circuit now calls `_back_to_sim_btn.configure(state=tk.NORMAL)`; BUG-C08 | — |
| REQ-21-20 | §21 GUI — Online Visualization legend includes ID-loss triangles | Legend row shows triangle swatches for Truck2/Truck3 × distance/scenario loss types on same row as line swatches | ✅ | 2026-05-31; 4 triangle entries added to `_build_legend` using `create_polygon` canvas swatches; BUG-C11 | — |
| REQ-21-21 | §21 GUI — Online Visualization braking info inline in leader row | Orange dist / Red dist / ID loss / ID resume labels appear to the right of Save As inside the leader control LabelFrame, not on a separate bar | ✅ | 2026-05-31; `_build_braking_info_bar` removed; labels inlined into `leader_row`; BUG-C11 | — |
| REQ-21-22 | §21 GUI — Scenario CSV initial gaps override | If scenario CSV metadata row 2 contains `Initial gaps:,<gap12>,<gap23>`, those values override `initial_gap_12_m` / `initial_gap_23_m` from the parameters file; falls back to parameters when row is absent | ✅ | 2026-06-01; `ScenarioTimeline.initial_gap_12_m/23_m` parsed in `from_csv`; `apply_initial_gaps()` called at all 4 simulation entry points (`simulation.py`, `optimization.py` ×2, `gui.py`) | — |
| REQ-21-23 | §21 GUI — controls row layout | "Open SimGenerator" button moved to controls row (left-aligned after bulk buttons); notification field + Copy button moved to tools row below | ✅ | 2026-06-01; `gui.py` controls/tools_row restructured | — |

...
## 2026-06-01 — Scenario CSV initial-gap override + GUI controls layout swap (REQ-21-22, REQ-21-23)

Two independent changes. (1) BUG-C12: bulk scenarios generated by SimGenerator embed per-scenario initial gaps in CSV row 2 (`Initial gaps:,<gap12>,<gap23>`), but `ScenarioTimeline.from_csv` discarded that row — every run started with the fixed gap from the parameters file instead. Fix: `ScenarioTimeline` now stores `initial_gap_12_m`/`initial_gap_23_m` (parsed from row 2 when present); a new `apply_initial_gaps(initial)` method returns overridden `InitialConditions`; called at all 4 entry points (`run_basic_simulation_from_files`, `run_stage_b_optimization_from_files`, bulk scan loop in `optimization.py`, `LiveSimStepper` init in `gui.py`). Falls back to parameter values when metadata row is absent. Smoke-tested against `SimGenerator/Scenarios/29052026_1739/1_3T_nV_mG_idL2qs_0ES.csv` — gaps 9.43/7.23 m read correctly. (2) GUI layout: "Open SimGenerator" button moved from `tools_row` (below controls) into the `controls` row, left-aligned after the bulk buttons; the notification status bar + Copy button moved down to `tools_row` where it spans full width. Files changed: `scenario_timeline.py`, `simulation.py`, `optimization.py`, `gui.py`.

Test suite: 107 tests; 4 failures + 33 errors pre-existing (fixture path mismatch). No new failures. All `test_scenario_timeline` tests pass.

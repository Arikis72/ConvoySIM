...
## 2026-05-31 (session 2) — Online Visualization: bug fixes, leader colors, profile resume, legend & layout (REQ-21-15 to REQ-21-21)

Fixed 8 bugs discovered during first live test of the Online Visualization. BUG-C04: back-step metric was `simulation_time_step_s` (10 sub-steps/s) instead of `output_resolution_s` (1 row/s), causing all live rows to be erased on a single back press — renamed to `_output_rows_per_second` and corrected. BUG-C05: Save As dropped truck2/truck3 events before the live-entry time because `timeline.rows` is velocity-only; injected event-only rows with carry-forward velocity before compression. BUG-C06/C10: leader upper rectangle used follower colors and ignored orange/red brake — added `leader_status_style()` in `visualization.py` using `is_orange_status`/`is_red_status` helpers; propagated `leader_command_kind` through `_build_output_row` and `_step_n` so orange/red brake states appear correctly in output rows. BUG-C07/C09: auto-advance used zero-acceleration HOLD and velocity convergence divided by `step_s` (0.1 s) instead of `return_to_sim_velocity_s` (3 s) — both corrected; added `return_to_sim_velocity_s: float = 3.0` to `SimulationParameters` (user-tunable). BUG-C08: Back to Sim button permanently disabled after undo-all + re-intervention because `_ensure_live_mode` short-circuit did not re-enable it — one-line fix. BUG-C11: legend row missing ID-loss triangle swatches; braking info on separate standalone bar — added 4 triangle polygon swatches to `_build_legend`; removed `_build_braking_info_bar` and inlined labels into leader row. Also fixed `CLAUDE.md` to require reading `AGENTS.md` with NO line limit, preventing partial reads that caused documentation to be skipped.

REQ IDs added: REQ-21-15 through REQ-21-21. Files changed: `gui.py`, `live_stepper.py`, `visualization.py`, `simulation.py`, `models.py`, `CLAUDE.md`.
